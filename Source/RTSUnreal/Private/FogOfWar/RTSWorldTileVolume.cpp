...
void ARTSWorldTileVolume::BeginPlay()
{
	...
	GridTileNumber = FMath::CeilToInt(FMath::Max(RTSWorldBounds.BoxExtent.X,RTSWorldBounds.BoxExtent.Y)) * 2;
	...
	for (int32 TileY = 0; TileY < GridTileNumber; ++TileY)
	{
		for (int32 TileX = 0; TileX < GridTileNumber; ++TileX)
		{
			FVector2D WorldPosition = FVector2D(
				TileWorldSize * TileX + TileWorldSize * 0.5f + GetActorLocation().X - HalfGridTileNumber * TileWorldSize,
                TileWorldSize * TileY + TileWorldSize * 0.5f + GetActorLocation().Y - HalfGridTileNumber * TileWorldSize);
